$(function() {
  ...
  $('#upload').uploadify({
    'uploader' : baseurl + '/assets/js/uploadify/uploadify.swf',
    'script' : siteurl + '/ajax/project/issueUploadAttachment',
    'scriptData' : {
      session : session,
      project_id : project,
      upload_token : upload_token
    },
    'cancelImg' : baseurl + '/assets/images/layout/icon-delete.png',
    'auto' : true,
    'multi' : true,
    'queSizeLimit' : 10,
  ...
  uploaded_attachments.find('.delete').live('click', function(){
    ...
    $.post(siteurl + '/ajax/project/issueRemoveAttachment', {
      filename : filename,
      upload_token : upload_token,
      project_id : project
    }, function(){
...
function init_sidebar_autocomplete(project){
  ...
  if(!autocomplete_sidebar_init){
    ...
    $.getJSON(siteurl + '/ajax/project/inactiveUsers?project_id=' + project, function(data){
      var suggestions = [];
      ...
      var input = $('#add-user-project');
       ...
       $(input).autocomplete({
        ...
        select: function (event, ui){
          ...
          $.post(siteurl + '/ajax/project/addUser', {
            user_id : ui.item.id,
            project_id : project
          }, function(data){
            saving_toggle();
...
function remove_project_user(user_id, project_id){
  ...
  $.post(siteurl + '/ajax/project/removeUser', {
    user_id : user_id,
...
function assign_issue_to_user(user_id, issue_id, callback){
   $.post(siteurl + '/ajax/project/issueAssign', {
      user_id : user_id,
      issue_id : issue_id
   }, function(){
      callback();
